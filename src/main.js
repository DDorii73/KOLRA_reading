...
산과 바다는 다른 점도 가지고 있는데 첫째, 산은 정상에 도착하기까지 올라가는 것이 힘들다. 그러나 바다는 바다를 바라보며 천천히 걸을 수 있어 힘이 들지 않는다. 그리고 산은 나무를 볼 수 있고 바다는 푸른 바다를 볼 수 있다는 것이 다르다. 또 산에서는 산새들과 곤충들을 볼 수 있고 바다에서는 갈매기와 바다생물들을 볼 수 있다는 것이 다르다. 이러한 이유로 사람들은 신중히 생각하고 자신이 가고 싶은 장소를 정한다. 그러나 우리나라는 산과 바다 모두 멋진 모습을 하고 있으며 그 나름대로의 아름다움을 가지고 있다.`
  },
  "grade-5-6": {
    title: "5,6학년 수준 [의생활]",
    level: "5,6학년 수준",
...
function attachTopicParticle(name) {
  const safeName = name || "학생 A";
  return `${safeName}${hasFinalConsonant(safeName) ? "은" : "는"}`;
}
...
function createReport({ formData, totalSyllables, errorSyllables, rate, comparison }) {
  ...
  const studentWithParticle = attachTopicParticle(studentName);
